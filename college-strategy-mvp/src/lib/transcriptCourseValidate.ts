/** Client-side mirror of server/transcriptCourseValidate.mjs for paste heuristic. */
...
export function isValidTranscriptGrade(grade: string): boolean {
...
export function isPlausibleCourseName(courseName: string): boolean {
...
export function isPlausibleCourseRow(courseName: string, grade: string): boolean {
...
export function sanitizeGpaValue(value: string, opts: { min?: number; max?: number } = {}): string {
  const { min = 1.5, max = 5.5 } = opts;
