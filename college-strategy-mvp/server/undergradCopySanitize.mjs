/** 本科报告文案：去除 Anderson/Wharton/Sloan 等研究生院系误表述 */
...
const ARRAY_FIELDS = ["key_fit_signals", "key_risks", "verification_focus"];

function normalizeSchoolNameInput(name) {
  return String(name || "")
    .trim()
    .replace(/^the\s+/i, "")
    .replace(/[—–\-_,]/g, " ")
...
const GLOBAL_GRAD_REPLACEMENTS = [
  {
    pattern: /UCLA\s+Anderson(\s+School(\s+of\s+Management)?)?/gi,
    zh: "UCLA 本科相关专业（Anderson 为研究生商学院）",
    en: "UCLA undergraduate paths (Anderson is graduate-only)",
...
/** @param {string} text @param {string} school @param {"zh"|"en"} locale */
export function sanitizeUndergradSchoolMentions(text, school, locale = "zh") {
  const s = String(text || "").trim();
  if (!s) return s;
  return applyRules(s, rulesForSchool(school), locale);
}

/** @param {Record<string, unknown>} row @param {"reach"|"match"|"safety"} tier @param {"zh"|"en"} locale */
export function sanitizeSchoolRowUndergradCopy(row, tier, locale = "zh") {
  ...
  for (const key of TEXT_FIELDS) {
    if (typeof out[key] === "string") {
      out[key] = sanitizeUndergradSchoolMentions(out[key], school, locale);
    }
  }
  for (const key of ARRAY_FIELDS) {
    if (Array.isArray(out[key])) {
      out[key] = out[key].map((x) => sanitizeUndergradSchoolMentions(String(x), school, locale)).filter(Boolean);
    }
  }
...
/** @param {string} blob */
export function containsUndergradFacultyErrors(blob) {
  return /UCLA\s+Anderson|强?\s*商学院\s*[\(（]?\s*Anderson|核实\s*Anderson|Anderson对|Haas\s*School|伯克利\s*Haas\s*商学院|(verify|核实)\s*(Anderson|Wharton|Sloan|Booth|Haas|Kellogg|Fuqua|Tepper|Tuck)/i.test(
    String(blob || ""),
  );
}
