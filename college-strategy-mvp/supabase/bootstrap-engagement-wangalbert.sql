-- Link counselor weiyiwang603@gmail.com ↔ student wangalbert816@gmail.com
-- Prerequisite: schema.sql + schema-crm.sql + bootstrap-counselor-weiyiwang.sql
-- Student must have at least one row in saved_applications (login → save a report in 我的申请)
...
begin
  ...
  from auth.users
  where lower(email) = lower('wangalbert816@gmail.com')
  order by created_at desc
  limit 1;

  if student_uid is null then
    raise exception 'No Auth user for wangalbert816@gmail.com. Student must sign in once (Google or email).';
  end if;
  ...
  if app_id is null then
    raise exception 'No saved_applications for wangalbert816@gmail.com. Student: generate a report while logged in, then retry.';
  end if;
  ...
  values (
    student_uid,
    'wangalbert816@gmail.com',
    split_part('wangalbert816@gmail.com', '@', 1),
    app_id,
  ...
  -- Seed demo content only when case is empty
